...
# ╔═╡ ef601719-2864-466e-83c1-0a6b14e9a977
begin
	using Plots
	using LinearAlgebra
end

# ╔═╡ 98648960-9338-11ec-1834-3110ccd46a8c
md"""
...
**Caso fixo-fixo** $\rightarrow$
condições de contorno: $f(0) = 0$ e $f(1) = 0$

Solução analítica aplicando as condições de contorno?
"""

# ╔═╡ 1ebcf2b1-1c83-4f0a-a2e2-7d73de1c4661
analiticoFF(x) = -x^2 + x

# ╔═╡ 13325a60-4c34-43e8-80ed-53629cd0e302
md"""
Dividindo a barra em 9 nós ($Δx = 0,125$, índices de 0 a 8), sendo o primeiro nó e o nono nó localizados nas extremidades e apresentando, pelas condições de contorno deslocamento nulo, pode-se escrever o problema no formato discreto:
...
$\frac{1}{(0,125)^2} \begin{bmatrix}
				...
				\end{bmatrix} \begin{bmatrix}
						f_1 \\ f_2 \\ f_3 \\ \vdots \\ f_7 \\
	  				\end{bmatrix} = \begin{bmatrix}
	   					2 \\ 2 \\ 2 \\ \vdots \\ 2 \\
		 			\end{bmatrix}$
"""

# ╔═╡ 333ca86d-084d-4717-88f9-871418a40385
begin
	Δx = 0.125
	b = fill(2,7)
...
# ╔═╡ 015e5edb-2c9c-48c9-a55c-00aba7684af5
solFF = K \ b

# ╔═╡ 84e1ebc1-32d8-46ca-a855-c6831247cafc
begin
	pushfirst!(solFF, 0)
	push!(solFF, 0)
end

# ╔═╡ dd04c390-1659-4c84-b689-663eedb62137
# Plotando resultados
begin
	plot(0:0.05:1, analiticoFF.(0:0.05:1), linecolor="red", linewidth=3, label="analítico")
	scatter!(0:Δx:1, solFF, markercolor="black", markersize=7, label="numérico")
end

# ╔═╡ 06de8c7d-1c82-4187-87e7-b784595d70c7
md"""
**Caso livre-fixo** $\rightarrow$
condições de contorno: $\frac{df}{dx}(0) = 0$ e $f(1) = 0$

Solução analítica?
"""

# ╔═╡ ef72fb18-46b7-4d44-b1ef-693f052bccd2
analiticoLF(x) = -x^2 + 1

# ╔═╡ 4bf76806-dbb5-49d4-91bc-2bed4e2292b1
md"""
...
$\frac{-f_2+2f_1-f_0}{Δx^2} = \frac{-f_2+2f_1-f_1}{Δx^2} = \frac{-f_2+f_1}{Δx^2} = 2$
...
# ╔═╡ 2b2abea8-7c0f-4382-adfc-82f50399db7a
begin
...
# ╔═╡ 38147544-eb8f-4d05-b09c-59e2f074207d
solLF = T \ b

# ╔═╡ 61de4d18-bd6d-43c9-b369-1a4afadc102e
begin
	pushfirst!(solLF, solLF[1])
	push!(solLF, 0)
end

# ╔═╡ a50c99c7-0bf0-42a2-ae48-79f2e942bfe3
# Plotando resultados
begin
	plot(0:0.05:1, analiticoLF.(0:0.05:1), linecolor="red", linewidth=3, label="analítico")
	scatter!(0:Δx:1, solLF, markercolor="black", markersize=7, label="numérico")
end

# ╔═╡ b96eb95e-595c-4191-a0e5-4cbe8e0adafd
md"""
**Por que encontramos o erro na extremidade livre??**
"""

# ╔═╡ c27f44c3-cb76-4e23-9823-9da4337b45f3
md"""
...
A matriz terá, portanto, uma linga adicional.

$\frac{-f_1 + 2f_0 - f_{-1}}{(Δx)^2} = \frac{-2f_1 + 2f_0}{(Δx)^2} =2$
...
# ╔═╡ 8bf05bc7-82db-46ac-8d9f-4531f31f8d73
begin
...
# ╔═╡ d680b67c-afcd-46f0-896f-d42ff4042db8
begin
	bb = fill(2,8)
	bb[1] = 1
end

# ╔═╡ d2e5cd67-6963-4d66-afa7-fa3463cd9c39
solLFb = Tb \ bb

# ╔═╡ 62142f87-cb47-4bda-bbb7-c4a7bdc25981
push!(solLFb, 0)

# ╔═╡ f5511cfa-05e1-4177-bcd7-fc133217aa1a
# Plotando resultados
begin
	plot(0:0.05:1, analiticoLF.(0:0.05:1), linecolor="red", linewidth=3, label="analítico")
...
# ╔═╡ 1c637ab7-c500-4955-83e0-d3530ea3d01f
md"""
**Diagrama de fase da solução analítica**
"""

# ╔═╡ b2b80f25-8b86-4d19-893c-ff6733925fef
begin
	plot(cos.(0:0.1:8), -sin.(0:0.1:8), linecolor="black", linewidth=3, label=false, ratio=1)
...
# ╔═╡ 5fc8e753-8178-4d3a-a3f4-b05a3f0784a9
md"""
...
# ╔═╡ 1df9c28d-dc18-49b4-b8b8-ed06cfac523a
...
# ╔═╡ a616e8aa-24cf-4055-8e17-bcddc80559bb
df = forwardEuler(0.1, 1.0, 0.0, 50)

# ╔═╡ 29315d07-dce7-41b2-ba13-8cf1d8e6667a
# Plotando dados
begin
	plot(cos.(0:0.1:8), -sin.(0:0.1:8), linecolor="black", linewidth=3, label=false, ratio=1)
	vline!([0], linecolor="gray", linestyle=:dash, label=false)
...
# ╔═╡ ac1f4995-dd07-4038-bb62-8f33389e05f5
md"""
Utilizando **Backward Euler**:

$\begin{cases}
	u_{n+1} = u_n + Δt \; w_{n+1} \\
 	w_{n+1} = w_n - Δt \; u_{n+1} \\
\end{cases}$
"""

# ╔═╡ 5447f889-06df-487a-9741-e4f3dae17a95
...
# ╔═╡ d66a95b2-ad61-4f9f-b202-e80865a949c3
db = backwardEuler(0.1, 1.0, 0.0, 50)

# ╔═╡ 670dc51e-634b-410c-a64a-ac5d7a916fdf
# Plotando dados
begin
	plot(cos.(0:0.1:8), -sin.(0:0.1:8), linecolor="black", linewidth=3, label=false, ratio=1)
	vline!([0], linecolor="gray", linestyle=:dash, label=false)
...
# ╔═╡ fad2be1f-b914-4223-b2cf-95c5891277cd
md"""
...
# ╔═╡ a43339e7-ccd9-4797-8f67-b48577cf28e1
...
# ╔═╡ 95c9aedd-8ffa-411d-ac0f-d079117bd3d1
dc = centeredDiff(0.1, 1.0, 0.0, 50)

# ╔═╡ ad66bbbf-8030-4096-b48b-8938c19def2e
# Plotando dados
begin
	plot(cos.(0:0.1:8), -sin.(0:0.1:8), linecolor="black", linewidth=3, label=false, ratio=1)
	vline!([0], linecolor="gray", linestyle=:dash, label=false)
