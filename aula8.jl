...
# ╔═╡ dea3199b-ccbc-4230-a07f-eea370947501
begin
	using LinearAlgebra
	using Plots
	using PlutoUI
...
# ╔═╡ 867cfc5e-94cc-11ec-3b00-7f9f51a6411f
md"""
...
# ╔═╡ eae24350-0bda-4064-8006-a1bec9225180
md"""
Vamos tentar resolver o problema numericamente.

$\frac{u_k^{n+1} - u_k^n}{Δt} = α\left[\frac{u_{k+1}^n - 2u_k^n + u_{k-1}^n}{(Δx)^2}\right]$

Acima vemos que a parcela no tempo é desenvolvida como *Forward Euler*.

$u_k^{n+1} - u_k^n = \frac{αΔt}{(Δx)^2}[u_{k-1}^n - 2u_k^n + u_{k+1}^n]$
...
# ╔═╡ 1e0bc961-8ce5-4879-bf6b-619be44300d7
fo(α, Δt, Δx) = α*Δt/(Δx)^2

# ╔═╡ 806228f0-55ec-4c6c-a917-6bbe7490f3aa
# Condições iniciais
begin
	u₀ = fill(0.0,11)
	u₀[1] = 20.0
...
$\begin{bmatrix}
		u_1^{n+1} \\ u_2^{n+1} \\ u_3^{n+1} \\ \vdots \\u_{k-1}^{n+1}
  \end{bmatrix} = 
  \begin{bmatrix}
  	u_1^n \\ u_2^n \\ u_3^n \\ \vdots \\ u_{k-1}^n \\
   \end{bmatrix}+ 
   Fo \begin{bmatrix}
		-2 & 1 & 0 & \cdots & 0 & 0 & 0 \\
 		1 & -2 & -1 & \cdots & 0 & 0 & 0 \\
...
# ╔═╡ 931ae6a2-6804-435b-84fa-a22a1b049f49
md"""
...
deps = ["Artifacts", "JLLWrappers", "Libdl", "Pkg"]
git-tree-sha1 = "ff35cb622ce29706b86c3750e9537f6182683570"
